...
class CellphoneScene extends Scene{
    ...
    public bindEvents():void{
        this.testVo = Timer.addTimer(3000, -1, () => {
            this.addOneMessage(Math.random() > 0.5, this.tests[Math.floor(Math.random() * this.tests.length)]);
        }, this);
    ...
    public addOneMessage(isMe:boolean, message:string):void{
        var entry: MsgEntry = new MsgEntry();
        var h: number = 0;
...
class MsgEntry extends egret.gui.Group{
    ...
    public addToMessage(group:egret.gui.Group, isMe: boolean, text: string): void{
